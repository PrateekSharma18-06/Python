...
allowed_names = ["lalit", "mohit"]

while True:
    acc_name = input('\nEnter Name of Account Holder to Login From (Lalit/Mohit) : ').lower()
    if acc_name in allowed_names:
        break
    else:
        print("\nInvalid name. Please enter either 'Lalit' or 'Mohit'.")
...
def update(dfp,xx,str_curr,str_init):
...
def chain(instrument_key,expiry_date,counter):
        ...
        global structure_initial, structure_current, past_data, initialize, xyz
...
def call():
    ...
    # print(f'Avg : {round(((prt0+prt1+prt2+prt3+prt4)/5)*60*30,2)} Req / 30 Min, (Max Rate = 2000 Request)\n')
    ...
    update(dfp,xx,str_curr,str_init)
